...
import random
import glob
from Itachi import app,BOT_USERNAME as MENTION_BOT,LOG
...
LOGOES = [
    ...
    "https://telegra.ph/file/74807bfbc85057899ea8d.jpg",
    ...
    "https://telegra.ph/file/4606e5c76a4a6c893a721.jpg",
    ...
    "https://telegra.ph/file/22a5d3621aba0b370d0b6.jpg",
    "https://telegra.ph/file/ae31845d1df2c4a84915b.jpg",
    "https://telegra.ph/file/ae2b809c8d11e7fa4121d.jpg",
    "https://telegra.ph/file/ccb7f3113994d5d2b26f6.jpg",
    "https://telegra.ph/file/5e53f0257ff12a7b0737a.jpg",
    ...
    "https://telegra.ph/file/d8d2db623223dee65963e.jpg",
    ...
    "https://telegra.ph/file/9e4bef8ae0725d6b62108.jpg",
...
async def logo_vai(logo_text,pic,link):
                if link != None:
                    randc = link
                ...
                fillcolor = "black"
                shadowcolor = "blue"
                fnt = glob.glob("./TeleBot/resources/fonts/*")
...
async def logo_make(_,message):
    ...
    if len(message.command) < 2: 
        await message.reply_text("**Give me a text to generate logo.**")
        return             
    logo_text = message.text.split(None,1)[1]
    text = await message.reply("**making.....**")        
    if not replied:
        try:
            randc = random.choice(LOGOES)
            final = await logo_vai(link=randc,logo_text=logo_text,pic=None)
            await pgram.send_photo(chat_id,final, caption=f"**Logo Generated By @{MENTION_BOT}**", reply_markup=key)
            await text.delete()
    ...
    if replied:
        file_id = await get_file_id_from_message(replied)
        if not file_id:
            return await message.reply_text('**Unsupported media.**')
        try:
            downloaded = await _.download_media(file_id)                
            final = await logo_vai(link=None,logo_text=logo_text,pic=downloaded)
            await pgram.send_photo(chat_id,final,caption=f"**Logo generated by @{MENTION_BOT}**", reply_markup=key)
            await text.delete()
            if os.path.exists(final):
                os.remove(final) 
...
♠ `/logo <text>` : create random logo with text.
♠ `/logo <text>` : create logo with your pic by tagging.
