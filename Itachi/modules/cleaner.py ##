...
from Itachi import ALLOW_EXCL, BOT_NAME, CustomCommandHandler, dispatcher
from Itachi.modules.disable import DisableAbleCommandHandler
from Itachi.modules.helper_funcs.chat_status import (
    bot_can_delete,
    connection_status,
    dev_plus,
    user_admin,
...
def add_bluetext_ignore(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        reply = "No command supplied to me ignored"
        message.reply_text(reply)
...
def bluetext_ignore_list(update: Update, context: CallbackContext):
    ...
    if global_ignored_list:
        text = "The following command are currently ignored globally from bluetext cleaning. :\n"

        for x in global_ignored_list:
            text += f" - <code>{x}</code>\n"

    if local_ignore_list:
        text += "\nThe following command are currently ignored locally from bluetext cleaning. :\n"

        for x in local_ignore_list:
            text += f" - <code>{x}</code>\n"

    if text == "":
        text = "No command supplied to be ignored."
...
__help__ = f"""
Bluetext cleaner removed any made up commands that people send in your chat..

• /cleanblue <on/off/yes/no>*:* `Clean command after sending`

• /ignoreblue <word>*:* `Prevent auto cleaning of the command`

• /unignoreblue <word>*:* `Remove prevent auto cleaning of the command `

• /listblue*:* `List currently whitelisted command`
 
*Only command, admin cannot use these:*
 
• /gignoreblue <word>*:* `Globally ignored bluetext cleaning of served word across` {BOT_NAME}.

• /ungignoreblue <word>*:* `Remove said command from global cleaning list`
"""

__mod_name__ = "Cleaner ♻️"
