...
def animalfact(update: Update, context: CallbackContext):
    update.effective_message.reply_text(random.choice(ANIMAL_FACTS))
...
ANIMALFACT_HANDLER = CommandHandler("animalfacts", animalfact, run_async=True)
dispatcher.add_handler(ANIMALFACT_HANDLER)
CAT_HANDLER = CommandHandler(("cats" ,"cat"), cats, run_async=True)
